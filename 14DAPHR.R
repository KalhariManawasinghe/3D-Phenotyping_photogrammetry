library(readr)
DAPHR <- read_csv("Desktop/Publication/14DAPHR.csv")
...
y_base <- max(DAPHR$HR) * 2

# Add equation and R² with smaller font
text(x = 6, y = max(DAPHR$HR) * 0.92, labels = equation_generic, col = "black", cex = 1.2, pos = 4)
text(x = 6, y = max(DAPHR$HR) * 0.85, labels = r2_text, col = "black", cex = 1.2, pos = 4)

library(readr)
CAFAGBMF <- read_csv("Desktop/Publication/CAFvsAGBMF.csv")

# Assume your data is in a data frame called '14CAFAGBMF'
# ConLAert Genotype to a factor (if it's not already)
CAFAGBMF$Genotype <- as.factor(CAFAGBMF$Genotype)

# Fit linear regression model
model <- lm(AGBMF ~ CAF, data = CAFAGBMF)
...
par(family = "Times")
# Plot data
plot(CAFAGBMF$CAF, CAFAGBMF$AGBMF,
...
y_base <- max(CAFAGBMF$AGBMF) * 2

# Add equation and R² with smaller font
text(x = 6, y = max(CAFAGBMF$AGBMF) * 0.70, labels = equation_generic, col = "black", cex = 1.5, pos = 4)
text(x = 6, y = max(CAFAGBMF$AGBMF) * 0.63, labels = r2_text, col = "black", cex = 1.5, pos = 4)

library(readr)
DPAV <- read_csv("Desktop/Publication/35DAPV.csv")
...
text(x = 5, y = max(DPAV$V) * 0.95, labels = equation_generic, col = "black", cex = 1.2, pos = 4)
text(x = 5, y = max(DPAV$V) * 0.86, labels = r2_text, col = "black", cex = 1.2, pos = 4)
